...
/**
 * The Realm Object Builder Generator is a processor that looks at all available Realm model classes
 * and create a companion builder class for constructing instances.
 */
@SupportedAnnotationTypes({
        "io.realm.annotations.RealmClass"
})
public class RealmObjectBuilderProcessor extends AbstractProcessor {
    ...
    private ClassData processClass(TypeElement classElement) {
        ...
        for (Element element : classElement.getEnclosedElements()) {
            ...
            if (elementKind.equals(ElementKind.FIELD)) {
                ...
                if (!ignoreField) {
                    data.addField(element.getSimpleName().toString(), getFieldType(element));
                }
            }
        }
     ...
     */
    private TypeName getFieldType(Element field) {
